...
// Mock blog data from the main website
const mockBlogPosts = [
  {
    id: '1',
...
async function importMockData() {
  try {
    ...
    const existingPosts = await db('blog_posts').select('*');
    if (existingPosts.length > 0) {
      console.log('⚠️  Blog posts already exist in database. Skipping import.');
    ...
    console.log('✅ Successfully imported', mockBlogPosts.length, 'blog posts');
    return { success: true, message: `Imported ${mockBlogPosts.length} blog posts` };
  } catch (error) {
